...
function EMG.update_objective(m, 𝒯, ℳ, modeltype::EMI.AbstractInvestmentModel)
    ...
    disc = Discounter(modeltype.r, 𝒯)               # Discount type declaration

    # Update of the cost function for modes with investments
    for t_inv ∈  𝒯ᴵⁿᵛ, tm ∈ ℳᴵⁿᵛ
        obj -= objective_weight(t_inv, disc, type="avg") * m[:capex_trans][tm, t_inv]
        obj -= objective_weight(t_inv, disc, type="avg") * m[:trans_opex_fixed][tm, t_inv] * t_inv.duration
        obj -= objective_weight(t_inv, disc,  type="avg") * m[:trans_opex_var][tm, t_inv] * t_inv.duration
    end
